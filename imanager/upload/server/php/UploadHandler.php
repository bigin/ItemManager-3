...
class UploadHandler
{
	...
	function init($options = null, $execute = true, $error_messages = null)
	{
		...
		$categoryid = (int) $this->imanager->input->get->categoryid;
		$fieldid = (int) $this->imanager->input->get->fieldid;
		...
		$this->options = array(
			...
			'user_dirs' => false,
			'mkdir_mode' => $this->imanager->config->chmodDir,
			'param_name' => 'files',
			...
			'accept_file_types' => '/\.('.(($field->configs->accept_types) ? $field->configs->accept_types :
					'gif|jpe?g|png').')$/i', //'/.+$/i',
			...
			'max_number_of_files' => (isset($field->configs->max_number_of_files) ? $field->configs->max_number_of_files : null),
			...
			'image_versions' => array(
				...
				*/
				'thumbnail' => array(
					...
					//'crop' => true,
					'max_width' => $this->imanager->config->thumbSize['width'],
					'max_height' => $this->imanager->config->thumbSize['height']
				)
			),
			'print_response' => true
		);
